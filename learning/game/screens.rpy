...
screen notebook_cell(cell, index):
    vbox:
        spacing 4
        
        # Input 區域
        hbox:
            text f"In [{cell.execution_count or ' '}]:" color "#999" size 14
            input value VariableInputValue("cell_source_" + str(index))
        
        # 執行按鈕
        textbutton "▶ Run":
            action Function(notebook.run_cell, index)
        
        # Output 區域
        if cell.outputs:
            hbox:
                text "Out:" color "#D63031" size 14
                vbox:
