...
from .cloudinary_upload import upload_avatar_to_cloudinary
import uuid
...
def profile_page(request):
    ...
    if request.method == "POST":
        ...
        if action == "choose_default":
            ...
            if uploaded_file:
                public_id = f"user_{request.user.id}_{uuid.uuid4().hex[:8]}"
                try:
                    cloudinary_url = upload_avatar_to_cloudinary(uploaded_file, public_id)
                    profile.avatar_url = cloudinary_url
                    profile.avatar_choice = "custom"
                    profile.save()
                except Exception as e:
                    messages.error(request, f"上傳圖片失敗: {str(e)}")
                    return render(request, "pages/profile.html", {
                        "profile": profile,
                        "default_avatars": DEFAULT_AVATAR_IDS,
                        "upload_error": f"上傳失敗：{str(e)}",
                    })
            return redirect("profile_page")
...
class RoleLoginView(LoginView):
    ...
    #登入成功後，優先導回 next 參數指定的頁面，沒有才用預設首頁
...
def _safe_student_file(filename):
    ...
    if not file_path.is_relative_to(submissions_dir) or file_path.suffix not in EDITABLE_STUDENT_FILE_TYPES:
