...
label chapter2_2_2:
    ...
    aria "恭喜你進入了額外的題目。"
    aria "請仔細觀察左邊紅框內浮現的程式碼，這裡執行後會出現什麼結果？請在接下來的主控台打出來。"

    # 4. 變更角色表情切換，並準備進入輸入環節
    hide aria talk
    show aria default:
        zoom 1.2
        right
...
label ch2_2_2_input:
        # 顯示背景與角色
    #scene question1
    show aria default at right



    aria "看！這是程式碼結構圖，接下來要根據下圖的程式碼，來解出正確的結果。"
    #"{image=question1}"
    show question1 with dissolve
    window show

    python:
        ans_2_2_2 = renpy.input("請輸入正確的 Python 正確的輸入結果：（輸入半形符號即可）", default="").strip()

    hide question1 with dissolve
    window auto
...
label chapter2_3:
        ...
        "【投影出現有問題的程式碼：】"
        show question5 with dissolve
        window show
        window auto

        menu:
            "缺少 User-Agent headers，伺服器拒絕非瀏覽器請求":
                hide question5 with dissolve
                window auto
                $ score += 10
                $ aria_trust += 1
...
label chapter3_1:
    ...
    "全息投影浮現出 ARIA 自動生成的 Matplotlib 程式码,但圖表的座標範圍(xlim)極度詭異。"
...
label ch3_1_input:
    ...
    if ans_3_1 == "assert":
        ...
        "【系統提示：扣除 15 分！目前分數：[score]，繼續加油，你可以的！】"
        jump ch3_1_input
...
label ch3_2_input:
            "【全息投影提示：請輸入正確的 os 函式來獲取檔案大小 (Bytes)】"#if os.path.______(video_path) > 0:\n    print('下載成功')
            
            show question7 with dissolve
            window show
...
label ch3_3_input:
            "【全息投影提示：請輸入防呆判斷，當 data 為什麼時給予預設值 26 度？】"#if data is ____:\n    data = 26
            
            show question8 with dissolve
            window show
...
label chapter4_2:
        ...
        label ch4_2_input:
            ...
            show question12 with dissolve
            window show

            python:
                ans_4_2 = renpy.input("請輸入要導入的多執行緒標準函式庫名稱（全部小寫）：", default="").strip()

            hide question12 with dissolve
            window auto
